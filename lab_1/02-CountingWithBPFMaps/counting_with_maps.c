...
void poll_stats(struct counting_with_maps_bpf *skel)
{
    ...
    int fd = bpf_map__fd(skel->maps.counters_map);
    ...
    while (true)
    {
        ...
        bpf_map_lookup_elem(fd, &key, &rec);

        /* TODO 5: print the number of packets received */
        log_info("Packet count: %llu", rec.packets_count);
        /* TODO 6: print the number of bytes received */
        log_info("Packet size: %llu", rec.packet_size);
        sleep(1);
    }
}
